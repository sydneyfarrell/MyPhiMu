...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Insert the new password and email into the Login table
    $stmt = $mysqli->prepare("INSERT INTO Login (email, password) VALUES (?, ?)");
    $stmt->execute([$email, $hashedPassword]);

    if ($stmt->rowCount() > 0) {
        // Password inserted successfully, redirect based on membership status
        ...
        echo "Failed to insert password. Please try again.";
    }
}
        ...
        <div class="card p-4 text-light bg-light mb-5">
            ...
            <div class="card-body w-100">
                <form name="login" action="" method="POST">
                    ...
                    <div class="input-group form-group mt-3">
                    <input type="password" class="form-control"
                            placeholder="verify password" name="verify_password">
                    </div>
