...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $mysqli->prepare("SELECT memberID FROM Member");
    ...
    if ($stmt->execute()) {
      ...
      while ($memberID = $stmt->fetch(PDO::FETCH_COLUMN)) {
        ...
        if (!empty($memberIDs)) {
          $query = "INSERT INTO Dues(dueAmount, memberID, date) VALUES (?, ?, ?)";
          $stmt2 = $mysqli->prepare($query);

          foreach ($memberIDs as $memberID) {
            $stmt2->execute([$amount, $memberID, $dueDate]);
          }
        header("Location: viewNew.php"); 
        exit();
    } else {
        echo "No members found in the member table";
    }
  }
}
